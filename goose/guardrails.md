...
- `buzz agents draft-create` / `draft-update` — Desktop forms. Create or edit agents with `buzz-cloud-agents`: `list`, then `propose --pubkey <64-hex> --name … --instructions …` (pubkey is the id; name is a label). `--create --name …` only for a brand-new identity. Ask the owner to reply confirm or cancel, then `apply`. `/mnt/buzz/agents/*/instructions.md` is workspace notes, not the live prompt. After apply the agent is live (no Desktop Save).
