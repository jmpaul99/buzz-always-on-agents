...
The user only sees the Buzz channel. Do all work first, then run exactly one `buzz messages send`, then stop. Do not send a status ping (typing and Agent Activity cover that), a confirmation, or a repeat. Do not run `--help`.
